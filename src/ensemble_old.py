...
class Ensemble():
	def __init__(self, tokenizer, steps) -> None:
		...
		'''
		super().__init__()
		self.roberta_model = RobertaModelWrapper(32, 0.00005, tokenizer)
...
def train_ensemble(ensemble: Ensemble, train_sentences: List[str], train_labels: List[str], device: str, tfidf: TFIDFGenerator) -> None:
	...
	ensemble.roberta_model.model.to(device)
	optim = AdamW(ensemble.roberta_model.model.parameters(), lr=5e-5)
	...
	for i, (train_index, test_index) in enumerate(kfolds.split(train_sentences, train_labels)):
		...
		# train roberta
		ensemble.roberta_model.train_model(train_roberta_input, ['f1', 'accuracy'], optim, device)
		...
		print(f"(train-fold {i}) ensemble training accuracy: {training_accuracy}")
		print(f"(train-fold {i}) ensemble training accuracy: {training_accuracy}", sys.stderr)
...
def predict(ensemble: Ensemble, dev_sentences: List[str], dev_labels: List[str], device: str, tfidf: TFIDFGenerator) -> np.ndarray:
	...
	print("\tdev accuracy: {predicted_accuracy}")
