...
def bertweet_io(model: AutoModel, sentences: List[str],
    ...
    # initialize bertweet tokenizer and pretrained model
...
def main(args: argparse.Namespace) -> None:
    ...
    print(f"({utils.get_time(start_time)}) Initalizating bertweet and creating data collator...\n")
    ...
    if args.model_folder != 'None':
        try:
            bertweet_model = AutoModel.from_pretrained(args.model_folder)
        ...
        bertweet_model = train_new_model(args, train_data, dev_data, data_collator,
                                        tokenizer, get_accuracy, start_time)

    # evaluate the model's performance
    print(f"\n({utils.get_time(start_time)}) Evaluating the Transformer model on training data\n")
    y_pred_train = evaluate(bertweet_model, args.batch_size, train_data, ['f1', 'accuracy'], device)

    print(f"\n({utils.get_time(start_time)}) Evaluating the Transformer model on dev data\n")
    y_pred_dev = evaluate(bertweet_model, args.batch_size, dev_data, ['f1', 'accuracy'], device)
    ...
    if args.save_file != 'None':
        bertweet_model.save_pretrained(args.save_file)
